    ...
    <div className="max-w-7xl mx-auto px-6 py-10">
      <div className="flex justify-between items-center mb-12">
        <div>
          ...
          <p className="text-zinc-400">
            Welcome back • Overview of your store
          </p>
        </div>

        <div className="text-xs px-5 py-2 bg-emerald-900/30 text-emerald-400 rounded-3xl font-mono">
          LIVE
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        {[
          ...
          {
            label: 'Active Users',
            value: (users.length + 1240).toString(),
            change: '+31%',
            color: 'sky',
          },
        ].map((stat, index) => (
          ...
          >
            <div className="text-xs text-zinc-400 tracking-wider mb-3">
              {stat.label}
            </div>

            <div className="text-5xl font-semibold text-white tracking-tighter mb-6">
              {stat.value}
            </div>

            <div
              className={`inline-flex text-xs items-center gap-1 ${
                stat.color === 'emerald'
                  ? 'text-emerald-400'
                  : stat.color === 'violet'
                  ? 'text-violet-400'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 bg-zinc-900 border border-zinc-800 rounded-3xl p-8">
          <div className="flex justify-between mb-8">
            ...
            <div className="text-xs self-end bg-zinc-800 text-zinc-400 px-4 py-2 rounded-2xl">
              LAST 6 MONTHS
            </div>
          </div>
          ...
          <div className="flex justify-between items-baseline mb-6 px-1">
            ...
            <div className="flex items-center gap-3">
              {isAdmin && (
                <Link
                  to="/create"
                  className="text-xs px-4 py-2 bg-emerald-600 text-white rounded-2xl hover:bg-emerald-500 transition"
                >
                  + CREATE
                </Link>
              )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {products.slice(0, 4).map((product) => (
              ...
              >
                <img
                  src={product.image}
                  className="w-20 h-20 object-contain bg-zinc-950 rounded-2xl"
                  alt=""
                />

                <div className="flex-1 pt-1">
                  <div className="text-sm font-medium line-clamp-2 leading-tight mb-3">
                    {product.title}
                  </div>
